...
namespace QL_quancafe
{
    public partial class MATHANG : MaterialForm
    {
        ...
        {
            DataTable HH = TasmaMain.LietKeDuLieu(
                "HANGHOA", "SoKho", TasmaMain.SK, TasmaMain.kn);
            txt_mahang.Text = TasmaMain.IDSinhTruong(
                (HH.Rows.Count > 0) ? HH.Rows[0]["MaHang"].ToString() : "HH000");
            txt_tenhang.Text = "";
            txt_dongia.Text = "";
            nup_soluong.Value = 0;
        }
        ...
        {
            DataTable MH = TasmaMain.LietKeDuLieu("MATHANG", TasmaMain.kn);
            MessageBox.Show(MH.Rows.Count.ToString());
